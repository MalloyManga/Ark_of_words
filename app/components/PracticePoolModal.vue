...
<template>
    ...
    <Teleport to="body">
        <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-emerald-950/30 px-5 py-6"
            ...
            <section
                class="font-zh-playful flex max-h-[min(82vh,42rem)] min-h-[min(82vh,42rem)] w-full max-w-2xl flex-col rounded-lg border border-emerald-200 bg-white p-5 text-left text-emerald-950 shadow-lg"
                role="dialog" aria-modal="true" aria-labelledby="practice-pool-title">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0">
                        <p id="practice-pool-title" class="text-xl font-black">
                            选择练习文本
                        </p>
                        <p class="mt-2 text-sm font-bold leading-6 text-emerald-950/65">
                ...
                <div class="scrollbar-hidden mt-5 flex-1 overflow-y-auto pr-1">
                    <div v-if="practicePool?.items.length" class="grid gap-2">
                        <button v-for="poolItem in practicePool.items" :key="poolItem.id" type="button"
                            ...
                            <div class="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1">
                                <span class="text-sm font-black text-emerald-950 sm:text-base">
                            ...
                            <div v-if="expandedPoolItemIds.has(poolItem.id)"
                                ...
                                <p class="wrap-break-word text-sm font-bold leading-6 text-emerald-700/80">
                    ...
                    <p v-else
                        class="rounded-md border border-emerald-100 bg-emerald-50/60 px-4 py-5 text-sm font-bold text-emerald-700/75">
                        暂无可用练习文本
                    </p>
                </div>
            </section>
